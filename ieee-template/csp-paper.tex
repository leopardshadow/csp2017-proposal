...
\author{My name$^{1}$ and Your Teammate's Name$^{2}$% <-this % stops a space
...
\begin{figure}[p] % t means put this image at the top 
...
The bot will follow a lane with Apriltag placed on various locations, each Apriltag has a plant beside it. The bot will move when it receives command from the central control unit, and report back when it completes.\\
...
With to position and facing direction of devices. We are able to plan the path of car. To move a plant, drive the car to the front of plant, turn car’s face to the plant, and start to load it.\\
